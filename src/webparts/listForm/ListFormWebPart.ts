...
> {
  ...
  public render(): void {
    ...
    if (Environment.type === EnvironmentType.Local) {
      ...
      element = React.createElement(ListForm, {
        ...
        onUpdateFields: (fields: IFieldConfiguration[]) =>
          this.updateField(fields),
      });
    } else {
      // show configure web part react component
      element = React.createElement(ConfigureWebPart, {
          ...
          try {
            if (!this.cachedLists) {
              return this.listService
                .getListsFromWeb(this.context.pageContext.web.absoluteUrl)
                .then((lists) => {
                  this.cachedLists = lists.map(
                    (l) => ({ key: l.url, text: l.title } as IDropdownOption)
                  );
  ...
  private formSubmitted(id: number) {
    if (this.properties.redirectUrl) {
      ...
      window.location.href = this.properties.redirectUrl.replace(
        "[ID]",
        id.toString()
      );
    }
