...
import { IFieldConfiguration } from "./fieldConfig/IFieldConfiguration";
...
import { FieldEditorComponent } from "./fieldConfig/FieldEditorComponent";
  ...
  public render() {
    ...
    if (this.state.fieldsSchema) {
      menuProps = {
        ...
        items: this.state.fieldsSchema.map((fld) => ({
        ...
        {this.state.isLoadingSchema ? (
          ...
          this.state.fieldsSchema && (
            <div>
              ...
              <div>
                <Modal
                  isOpen={this.state.editingField}
                  isModeless={true}
                  isBlocking={true}
                  isDarkOverlay={true}
                >
                  <FieldEditorComponent context={this.props.context} />
                </Modal>
              </div>
            </div>
          )
            ...
            if (fieldComponent && this.props.inDesignMode) {
              return (
                <DraggableComponent
                  ...
                  index={idx}
                  itemKey={field.key}
                  moveField={(dragIdx, hoverIdx) =>
                    this.moveField(dragIdx, hoverIdx)
                  }
  ...
  private validator = () => {
    ...
    this.state.fieldsSchema.forEach((currentFieldSchema) => {
      if (
        ...
        !this.state.data[currentFieldSchema.InternalName]
      ) {
        fieldErrors = {
          ...fieldErrors,
    ...
    if (shouldSave) {
      ...
      try {
        ...
        if (hadErrors) {
          ...
          if (id === 0) {
            id = updatedValues.filter((val) => val.FieldName === "Id")[0]
              .FieldValue;
          }
